...
- [ ] `PageResponse<T>` 공통 DTO (Spring `Page` 직접 반환 금지)
- [ ] `GET /api/todos` — page, size, sort, completed 필터, keyword 검색
- [ ] `POST /api/todos`, `GET/PATCH/DELETE /api/todos/{id}`
- [ ] `PATCH /api/todos/{id}/toggle`
- [ ] 전 경로 소유권 스코프 (`findByIdAndUserIdAndDeletedAtIsNull`, 실패 시 404)
- [ ] Soft Delete 처리
- [ ] description HTML sanitize (**jsoup 의존성 추가**, 허용 태그 화이트리스트 — `API_SPEC.md` 3.2)
- [ ] Swagger 어노테이션
...
- [ ] **패키지 설치**: `react-hook-form`, `zod`, `@hookform/resolvers`, `@tiptap/*`, `motion`
      (`PRD.md` 1.3 — M5 도입)
- [ ] `/` 랜딩 — 서비스 소개. 로그인 상태면 `/todos` 리다이렉트
      (**쿠키를 JS로 읽지 않는다** — 서버 사이드 판정 또는 `GET /api/auth/me` 결과로 판정)
- [ ] `/signup`, `/login` — 폼 검증(Zod, 비밀번호 `.min(6)`), 서버 에러 표시, 구글 버튼
- [ ] `/oauth2/callback` — **리다이렉트 경유 화면**. 쿠키는 백엔드가 이미 심었으므로
      토큰을 받거나 저장하지 않는다. 성공 → `/todos`, 실패 → `/login?error=oauth`
- [ ] `/todos` 목록 + Pagination 연결
- [ ] 완료/미완료 필터, 키워드 검색
- [ ] `/todos/new` — **별도 작성 페이지** (제목, 마감일, 우선순위, Tiptap 설명)
- [ ] `/todos/[id]` — **별도 상세·편집 페이지**. 다이얼로그·인라인 수정이 아니다
      (`PRD.md` 7장 확정. `[id]` URL 이 있어야 FR-T13 을 URL 레벨에서 검증할 수 있다)
- [ ] 완료 토글 — 낙관적 업데이트 + 롤백
- [ ] 삭제 — 확인 절차 + 낙관적 업데이트 + 롤백
- [ ] Tiptap 클라이언트 전용 렌더링 (`immediatelyRender: false`)
- [ ] 로딩 / 에러 / 빈 상태 3종
- [ ] motion 애니메이션 (150~250ms)
- [ ] 반응형 320 / 768 / 1280 확인
