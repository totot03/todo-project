...
| `sort`      | string  | `createdAt,desc` | `createdAt,desc`(최신순) 또는 `createdAt,asc`(오래된순)만 허용 — 그 외 값은 기본값으로 대체된다. 정렬 필드는 `createdAt`으로 고정이며 다른 필드는 지원하지 않는다 (FR-T06) |

예: `GET /api/todos?page=0&size=10&completed=false&keyword=회의&sort=createdAt,asc`
...
- 정렬 필드는 **생성일(`createdAt`)로 고정**이며, `sort` 파라미터로 방향(최신순/오래된순)만 선택할 수 있다 (FR-T06).
